...
public class AppMsgHandler implements HttpHandler {
    ...
    public void handle(HttpExchange t) throws IOException {
        System.out.printf((Const.Log_Receive_Msg) + "%n", this.app.getAppName());
    ...
    class HandlerThread extends Thread{
        private final YDApp app;
        private final AppClient appClient;
        private final IReceiveYdMsg receiver;
        ...
        private void handlerReq(){
            try {
                ...
                if(buin != this.app.getBuin()){
                    System.out.printf((Const.Log_Receive_Msg_Error_Invalid_BuinUnequal) + "%n", this.app.getAppName(),buin, this.app.getBuin());
                    return;
                }
                if(!appId.equals(this.app.getAppId())){
                    System.out.printf((Const.Log_Receive_Msg_Error_Invalid_AppIdUnequal) + "%n", this.app.getAppName(),appId, this.app.getAppId());
                    return;
                }
                boolean fromYD = Helper.signatureIsValid(reqParam, this.app.getToken(), encrypt);
                if(!fromYD){
                    System.out.printf((Const.Log_Receive_Msg_Error_Invalid_Signature) + "%n",this.app.getAppName());
                    return;
                }
